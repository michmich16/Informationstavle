const urls = {
  ...
  Promise.all([
    ...
    .then(([news, busTimes, weather, kantine, aktivitets]) => {
      ...
      displayData(
        busTimes.MultiDepartureBoard.Departure.map(
      ...
      const filteredAktivitets = aktivitets.value.filter(
        (val) => val.StartDate.split("T")[0] === currentDateFormatted
      );
      displayData(
        filteredAktivitets
          .map(
            (val) =>
              ` ${val.Room} - ${val.Education} -${formatDateToDaninsh(
                val.StartDate
              )} - ${val.Subject}`
          )
          .join("<span>"),
